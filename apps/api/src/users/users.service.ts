...
export class UsersService extends TypeOrmCrudService<User> {
  ...
  private NGINX_CONTROL_BAN_URL = this.configService.get('NGINX_CONTROL_URL')+'/drop/client';
  ...
  setInactiveAndEndStream(id: string) {
    return from(this.repo.findOne(id)).pipe(
      ...
      exhaustMap(user => this.http.get(this.NGINX_CONTROL_BAN_URL+'?app=app&name='+user.name, {headers: {Authorization: 'Basic '+this.configService.get('NGINX_CONTROL_AUTH_BASE64')}}))
    );
  }
